function [nl_struct,raster_struct, result] = nplick_struct_anesthesia(dirlist_root, varargin)
    ...
    for i=1:numel(dirlist)     
       ...
       % working_buff:  spout position = 1, analog lick raw = 2, contingency = 3, dispense = 4, opto = 5, lick = 6, reward live =
       % 7, rwliveonset = 8   
       ...
       nl_struct(i).np_pairs = sensor_on_off_times(working_buff(:,3));
       nl_struct(i).dispense = sensor_on_off_times(working_buff(:,4));
       nl_struct(i).laser_on = sensor_on_off_times(working_buff(:,5));
       nl_struct(i).lick_pairs = sensor_on_off_times(y);         
       nl_struct(i).rw_live = sensor_on_off_times(working_buff(:,7));             
       ...
       try
           ...
           %    .dat file (rw_cue)
           nl_struct(i).rw_cue = sensor_on_off_times(working_buff(:,8));
