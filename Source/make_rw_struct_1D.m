function [rw_trial_struct] = make_rw_struct_1D(nl_struct)
rw_struct_number = 1;
trial_num = 1;
...
for i = 1:numel(nl_struct)
    if (isempty(nl_struct(i).rw_cue)==0)
        ...
        actuator1_AP = nl_struct(i).actuator1_AP;
        actuator1_AP_command = nl_struct(i).actuator1_AP_command';
        ...
        for j =1:length(rw_cue(:,1))
            ...
            rw_trial_struct(trial_num).actuator1_AP = actuator1_AP(j);
            rw_trial_struct(trial_num).actuator1_AP_command = actuator1_AP_command(rw_cue(j,1):rw_cue(j,2));
            ...
            if(isempty(dispense)==0)
                ...
                lick_offset_in_cue = lick_offset((lick_offset>rw_cue(j,1))&(lick_offset<(rw_cue(j,1)+1300)));
                lick_offset_in_cue = lick_offset_in_cue - rw_cue(j,1);
                ...
                try
                   ...
                   if numel(licks_in_cue)
                    ...
                    rw_trial_struct(trial_num).rw_licks_offset = [];
