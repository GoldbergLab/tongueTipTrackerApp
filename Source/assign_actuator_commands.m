function t_stats = assign_actuator_commands(t_stats,l_sp_struct)
...
for i=1:numel(t_stats)
    if t_stats(i).lick_index == 1
        ...
        %   vector to this lick.
        t_stats(i).actuator_command_x = l_sp_struct(t_stats(i).trial_num).actuator1_ML_command;
        t_stats(i).actuator_command_y = l_sp_struct(t_stats(i).trial_num).actuator2_AP_command;
    else
